...
public sealed class Result<TValue> : IResult<TValue>
{
    ...
    /// <remarks>This constructor is obsolete and intended for use by serializers only.</remarks>
    [JsonConstructor]
    ...
    private Result(List<Error> errors)
    {
        if(errors is null || errors.Count == 0)
        {
            throw new ArgumentException(
                "Cannot create an Error of <TValue> from an empty collection or errors provide at least one error",nameof(errors));
        }
        _errors = errors;
        IsSuccess = false;
    }
